...
export const findMealPlansByDateRange = async (userId, startDate, endDate) => {
  const result = await db.query(
    `SELECT 
      mp.id, mp.user_id, mp.recipe_id,
...
export const getWeeklyMealPlan = async (userId, weekStartDate) => {
  const endDate = new Date(weekStartDate);
  endDate.setDate(endDate.getDate() + 6);

  return await findMealPlansByDateRange(userId, weekStartDate, endDate);
};

export const getUpcomingMeals = async (userId, limit = 5) => {
  const result = await db.query(
...
export const getMealPlanStats = async (userId) => {
  const result = await db.query(
    `SELECT 

...
export const generateShoppingListFromMealPlan = async (
  ...
  try {
    ...
    return await getShoppingListByUserId(userId); // ✅ fixed
  } catch (err) {
...
export const createShoppingItem = async (userId, itemData) => {
  const {
    ingredient_name,
    quantity,
    unit,
...
export const getShoppingListGrouped = async (userId) => {
  const result = await db.query(
    `SELECT category,
      json_agg(
        json_build_object(
...
export const updateShoppingItem = async (id, userId, updates) => {
...
export const toggleShoppingItem = async (id, userId) => {
  const result = await db.query(
...
export const deleteShoppingItem = async (id, userId) => {
  const result = await db.query(
    `DELETE FROM shopping_list_items
     WHERE id = $1 AND user_id = $2
     RETURNING *`,
...
export const clearCheckedItems = async (userId) => {
  const result = await db.query(
    `DELETE FROM shopping_list_items
...
export const clearAllShoppingItems = async (userId) => {
  const result = await db.query(
    `DELETE FROM shopping_list_items
     WHERE user_id = $1
     RETURNING *`,
...
export const addCheckedItemsToPantry = async (userId) => {
  const client = await db.pool.connect();
